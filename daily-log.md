...
**Thoughts**: The Patatap clone was interesting and a little confusing at first. Reading documentation is obviously a very important skill that will take a while to get used to. In the end it all made sense and I understand all of the code. During the Drumkit exercise I realized I really have a long ways to go on learning all of CSS. knowing the right CSS properties to use is a daily battle, but I am learning more of them everyday. During the firs watchthrough of the JS video there were a few new things, especially the data- element and how he selected the data- element with querySelector using back tics and ${} which ended up looking like `const audio	= document.querySelector(` ``audio[data-key="${e.keyCode}"]`` `)`. I was able to figure out about 50% of the code and had to follow along for the other 50%. Learned a lot about CSS transform. All in all it was fun and as a drummer it was a very fun build.


**Link(s) to work**: [CodePen Patatap Clone Link](https://codepen.io/ross-svh-code/full/JverjV/), [CodePen Drum Kit Link](https://codepen.io/ross-svh-code/full/NMEMjx/)
